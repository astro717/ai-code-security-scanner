/**
 * E2E test for CLI scanning of Rust (.rs) files.
 *
 * Compiles the TypeScript source, then invokes dist/cli.js against
 * tests/fixtures/vulnerable.rs. Asserts that Rust-specific findings
 * (BUFFER_OVERFLOW, COMMAND_INJECTION, WEAK_CRYPTO, SECRET_HARDCODED, INSECURE_RANDOM)
...
const FIXTURE = path.join(PROJECT_ROOT, 'tests', 'fixtures', 'vulnerable.rs');

describe('CLI E2E — Rust scanner', () => {
  beforeAll(() => {
  ...
  test('exits with code 1 and emits JSON findings for vulnerable.rs', () => {
  ...
  test('detects expected Rust finding types', () => {
    const result = spawnSync(
      process.execPath,
      [DIST_CLI, FIXTURE, '--json'],
      {
