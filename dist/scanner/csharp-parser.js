...
exports.parseCSharpCode = parseCSharpCode;
exports.scanCSharp = scanCSharp;
const fs = __importStar(require("fs"));
function parseCSharpFile(filePath) {
    const code = fs.readFileSync(filePath, 'utf-8');
    return parseCSharpCode(code, filePath);
}
function parseCSharpCode(code, filePath = 'input.cs') {
...
const CSHARP_PATTERNS = [
    // SQL injection via string interpolation/concatenation in ADO.NET
    {
        ...
        message: 'SqlCommand constructed with string interpolation or concatenation. User input in SQL ' +
    ...
    },
    {
        ...
        message: 'CommandText set with string interpolation or concatenation. Use parameterised queries ' +
    ...
    },
    {
        ...
        message: 'Entity Framework raw SQL called with string interpolation or concatenation. ' +
    ...
    // Command injection via Process.Start
    {
        ...
        message: 'Process.Start() called with user-controlled input. This allows arbitrary command ' +
    ...
    },
    {
        ...
        message: 'ProcessStartInfo constructed with user-controlled input. Validate all arguments ' +
    ...
    // Hardcoded secrets
    {
        ...
        message: 'Potential hardcoded credential in C# source. Secrets must be loaded from environment ' +
    ...
    // Weak crypto
    {
        ...
        message: 'Weak or broken cryptographic algorithm used (.Create()). ' +
    ...
    },
    {
        ...
        message: 'Deprecated cryptographic service provider instantiated. These algorithms are cryptographically ' +
    ...
    // Path traversal
    {
        ...
        message: 'File or directory operation with user-controlled input. Without path canonicalisation, ' +
    ...
    // Insecure random
    {
        ...
        message: 'System.Random is not cryptographically secure and must not be used for security-sensitive ' +
    ...
    // Unsafe deserialization
    {
        ...
        message: 'BinaryFormatter is unsafe and banned in .NET 5+. Deserializing untrusted data with ' +
    ...
    // XSS via Response.Write in ASP.NET
    {
        ...
        message: 'User input written directly to HTTP response via Response.Write() without HTML encoding. ' +
    ...
    // SSRF via HttpClient/WebClient with user input
    {
        ...
        message: 'HTTP request made with user-controlled URL. Without URL validation, attackers can force ' +
    ...
    // Open redirect
    {
        ...
        message: 'Response.Redirect() called with user-controlled input. This can be exploited for phishing ' +
    ...
    // C# unsafe block — managed memory-safety is suspended
    {
        ...
        severity: 'medium',
        pattern: /\bunsafe\s*\{/,
        message: 'C# unsafe block detected — managed memory-safety guarantees (bounds checking, null reference protection) ' +
...
function scanCSharp(result) {
        ...
        if (!inLoop && LOOP_ENTRY.test(line)) {
            ...
            if (loopBraceDepth <= 0) {
                ...
                findings.push({
                    ...
                    message: 'Database query inside a foreach/for loop — this is an N+1 query pattern. ' +
    ...
    let recentAttributes = []; // accumulates attributes before a method
            ...
            if (attrMatch) {
                ...
                if (!hasAuth) {
                    findings.push({
                        ...
                        column: line.search(/\S/),
                        snippet: trimmed.slice(0, 100),
                        message: 'ASP.NET controller action missing [Authorize] attribute — this endpoint is accessible ' +
