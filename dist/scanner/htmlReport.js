...
const REMEDIATION_SNIPPETS = {
    SQL_INJECTION: `// Use parameterised queries instead:\nconst result = await db.query(\n  "SELECT * FROM users WHERE id = $1",\n  [userId]\n);`,
    COMMAND_INJECTION: `// Use execFile with explicit args (no shell):\nimport { execFile } from 'child_process';\nexecFile('ls', ['-la', dir], callback);`,
    SHELL_INJECTION: `// Avoid shell=true; pass args as array:\nsubprocess.run(['cmd', arg1, arg2])`,
    EVAL_INJECTION: `// Replace eval() with a safe parser:\nconst data = JSON.parse(input);\n// Or use a sandboxed interpreter`,
    XSS: `// Escape output before rendering:\nconst safe = escapeHtml(userInput);\nelement.textContent = safe;  // not innerHTML`,
    SECRET_HARDCODED: `// Load from environment variable:\nconst apiKey = process.env.API_KEY;\n// Or use a secrets manager (Vault, AWS SSM)`,
    PATH_TRAVERSAL: `// Resolve and validate the path:\nconst resolved = path.resolve(baseDir, userInput);\nif (!resolved.startsWith(baseDir)) throw new Error('Invalid path');`,
    SSRF: `// Validate URL against allowlist:\nconst url = new URL(input);\nif (!ALLOWED_HOSTS.includes(url.hostname)) throw new Error('Blocked');`,
    OPEN_REDIRECT: `// Validate redirect is relative or allowlisted:\nif (!url.startsWith('/') || url.startsWith('//')) throw new Error('Invalid redirect');`,
    ...
    INSECURE_RANDOM: `// Use crypto.randomBytes for security:\nimport { randomBytes } from 'crypto';\nconst token = randomBytes(32).toString('hex');`,
    JWT_HARDCODED_SECRET: `// Load JWT secret from env:\nconst secret = process.env.JWT_SECRET;\njwt.sign(payload, secret, { algorithm: 'HS256' });`,
    JWT_NONE_ALGORITHM: `// Always specify allowed algorithms:\njwt.verify(token, secret, { algorithms: ['HS256'] });`,
    JWT_DECODE_NO_VERIFY: `// Use jwt.verify() instead of jwt.decode():\nconst payload = jwt.verify(token, secret);`,
    UNSAFE_DESERIALIZATION: `// Use safe serialization (JSON):\nconst data = JSON.parse(input);\n// Never use pickle.loads() or BinaryFormatter on untrusted data`,
    BUFFER_OVERFLOW: `// Use bounded functions:\nfgets(buf, sizeof(buf), stdin);\nsnprintf(buf, sizeof(buf), "%s", input);`,
    FORMAT_STRING: `// Always use a literal format string:\nprintf("%s", user_input);\n// Never: printf(user_input);`,
    MASS_ASSIGNMENT: `# Explicitly list permitted attributes:\nparams.require(:user).permit(:name, :email)`,
