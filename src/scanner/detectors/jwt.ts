...
function getStringValue(node: TSESTree.Node): string | null {
  if (isStringLiteral(node)) return (node as TSESTree.Literal).value as string;
  return null;
}
 ...
 */
function getSecretArgNode(call: TSESTree.CallExpression): TSESTree.Node | null {
  if (call.arguments.length < 2) return null;
  const arg = call.arguments[1];
  if (arg.type === 'SpreadElement') return null;
  return arg as TSESTree.Node;
}

export function detectJWTSecrets(result: ParseResult): Finding[] {
  ...
  walkNode(result.ast, (node) => {
    ...
    const isMemberSign =
      ...
      !(call.callee as TSESTree.MemberExpression).computed &&
      (call.callee as TSESTree.MemberExpression).property.type === 'Identifier' &&
      ((call.callee as TSESTree.MemberExpression).property as TSESTree.Identifier).name === 'sign';

    // Also match bare sign() — less common but possible after destructuring
    const isBareSign =
      call.callee.type === 'Identifier' &&
      (call.callee as TSESTree.Identifier).name === 'sign';

    if (!isMemberSign && !isBareSign) return;
    ...
    if (secretNode.type === 'ObjectExpression') {
      const obj = secretNode as TSESTree.ObjectExpression;
