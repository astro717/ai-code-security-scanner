 ...
 */
function loadAiScannerIgnore(workspaceRoot) {
...
function isIgnoredByAiScanner(filePath, patterns) {
    return patterns.some((p) => matchesGlob(filePath, p));
}
// ── HTTP helper ───────────────────────────────────────────────────────────────
function postJSON(serverUrl, body, apiKey) {
    return new Promise((resolve, reject) => {
        const payload = JSON.stringify(body);
        const parsed = new url_1.URL(serverUrl + '/scan');
...
function findingsToDiagnostics(findings) {
    return findings.map((f) => {
        ...
        const diag = new vscode.Diagnostic(range, `[${f.severity.toUpperCase()}] ${f.type}: ${f.message}`, severity);
...
// ── Scanner ───────────────────────────────────────────────────────────────────
async function scanDocument(document, collection) {
    ...
    try {
        ...
        vscode.window.setStatusBarMessage(`$(warning) AI Security Scanner: server unreachable — ${msg}`, 5000);
        return;
    }
    const diagnostics = findingsToDiagnostics(response.findings);
    collection.set(document.uri, diagnostics);
    if (diagnostics.length > 0) {
        vscode.window.setStatusBarMessage(`$(shield) AI Security: ${response.findings.length} finding(s) in ${vscode.workspace.asRelativePath(document.uri)}`, 6000);
    }
    else {
        vscode.window.setStatusBarMessage(`$(shield) AI Security: clean — ${vscode.workspace.asRelativePath(document.uri)}`, 3000);
    }
}
async function scanWorkspace(collection) {
    ...
    const filteredFiles = ignorePatterns.length > 0
        ? files.filter((uri) => !isIgnoredByAiScanner(uri.fsPath, ignorePatterns))
        : files;
...
function updateStatusBar(item, state, detail) {
    switch (state) {
        case 'idle':
            item.text = '$(shield) AI Sec';
...
function activate(context) {
    ...
    async function scanDocumentWithStatus(document) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('aiSecScan.scanFile', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('aiSecScan.scanWorkspace', () => {
